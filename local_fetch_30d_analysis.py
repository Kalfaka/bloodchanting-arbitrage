...
class TopItemsAnalyzer:
    ...
    def save_results(self, fetch_results: Dict, all_trades: List[Dict],
        ...
        cache_file = CACHE_DIR / f"raw_trades_{self.timestamp}.json"
        cache_data = {
            "metadata": {
                "timestamp": datetime.now().isoformat(),
                "window_days_primary": MAX_DAYS_HISTORY,
                "window_days_fallback": EXTENDED_DAYS_HISTORY,
                "total_trades": len(all_trades),
                "items_fetched": len(fetch_results),
                "items_with_trades": sum(1 for r in fetch_results.values() if r['trade_count'] > 0),
                "items_using_90d_fallback": sum(1 for r in fetch_results.values() if '90d' in r['fetch_window']),
                "api_url": API_URL,
                "volatility_threshold": self.analyzer.threshold,
                "min_trades_threshold": MIN_TRADES_THRESHOLD,
                "retry_stats": self.fetcher.retry_stats
            },
            "trades": all_trades
        }
        ...
        print(f"✓ Raw trade cache: {cache_file.name}")
        ...
        print(f"✓ Top items list: {OUTPUT_TOP_ITEMS.relative_to(SCRIPT_DIR)}")
        print(f"  Size: {top_items_size:.2f} KB")
        
        summary_file = SCRIPT_DIR / "analysis_summary.txt"
        with open(summary_file, 'w') as f:
            f.write("="*80 + "\n")
            f.write("VOLATILITY-AWARE TRADE ANALYSIS SUMMARY (ROBUST MODE)\n")
            f.write("="*80 + "\n")
            f.write(f"Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write(f"Primary Window: Last {MAX_DAYS_HISTORY} days\n")
            f.write(f"Fallback Window: Last {EXTENDED_DAYS_HISTORY} days (when 0 trades in 30d)\n")
            f.write(f"\n")
            f.write(f"VOLATILITY PARAMETERS:\n")
            f.write(f"  Threshold: {self.analyzer.threshold:.1f}%\n")
            f.write(f"  Minimum trades per window: {MIN_TRADES_THRESHOLD}\n")
            f.write(f"\n")
            f.write(f"DATA COLLECTION:\n")
            f.write(f"  Total items fetched: {len(fetch_results)}\n")
            f.write(f"  Total trades: {len(all_trades):,}\n")
            f.write(f"  Items analyzed: {analysis_stats['total_analyzed']}\n")
            f.write(f"  Items using 90-day fallback: {sum(1 for r in fetch_results.values() if '90d' in r['fetch_window'])}\n")
            f.write(f"  Total retries: {self.fetcher.retry_stats['total_retries']}\n")
            f.write(f"\n")
            f.write(f"WINDOW USAGE:\n")
            f.write(f"  24h window: {analysis_stats['window_usage']['24h']} items\n")
            f.write(f"  7d window: {analysis_stats['window_usage']['7d']} items\n")
            f.write(f"  30d window: {analysis_stats['window_usage']['30d']} items\n")
            f.write(f"\n")
            f.write(f"TOP ITEMS SELECTED:\n")
            f.write(f"  Blood Shard items: {len(top_shard)}\n")
            f.write(f"  Blood Token items: {len(top_token)}\n")
            f.write(f"  Total: {len(item_names)}\n")
            f.write(f"\n")
            f.write("="*80 + "\n")
        
        summary_size = summary_file.stat().st_size / 1024
        print(f"✓ Analysis summary: {summary_file.name}")
        print(f"  Size: {summary_size:.2f} KB")
        
        print(f"\n{'='*80}")
        print(f"NEXT STEPS:")
        print(f"{'='*80}")
        print(f"1. Review {summary_file.name} for analysis details")
        print(f"2. Upload data/top_items.json to GitHub when ready")
        print(f"3. All data collected successfully with {self.fetcher.retry_stats['total_retries']} retries!")
        print(f"{'='*80}\n")

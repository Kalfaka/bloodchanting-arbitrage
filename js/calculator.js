...
export function formatMillions(millionsValue) {
  ...
  // For values < 1M, show up to 3 significant decimals (e.g., 0.13M, 0.006M)
  // But don't show unnecessary trailing zeros
  const fixed2 = millionsValue.toFixed(2);
  if (parseFloat(fixed2) > 0) {
    return fixed2 + 'M';
  }

  // Very small values, show 3 decimals
  return millionsValue.toFixed(3) + 'M';
}
...
export class ArbitrageCalculator {
  ...
  calculateStatistics(trades) {
    ...
    // FIX: Use per-unit price, not total trade value (don't multiply by amount)
    const pricesGP = trades.map(t => this.api.convertToGPPerUnit(t));
...
export class BloodchantingCalculator {
  ...
  calculateScenario(scenario, shardItems, tokenItems) {
    // Find the cheapest way to get the shards we need
    // This is NOT necessarily the item with lowest cost per shard,
    // because you have to buy whole items
    const cheapestShardItem = shardItems.reduce((best, item) => {
      const shardsPerItem = item.tokens_received;
      const itemsNeeded = Math.ceil(this.RECIPE.blood_shards / shardsPerItem);
      const actualCost = itemsNeeded * (item[`${scenario}_price_gp`] / 1_000_000);

      const bestShardsPerItem = best.tokens_received;
      const bestItemsNeeded = Math.ceil(this.RECIPE.blood_shards / bestShardsPerItem);
      const bestActualCost = bestItemsNeeded * (best[`${scenario}_price_gp`] / 1_000_000);

      return actualCost < bestActualCost ? item : best;
    });

    // Find the cheapest way to get the tokens we need
    const cheapestTokenItem = tokenItems.reduce((best, item) => {
      const tokensPerItem = item.tokens_received;
      const itemsNeeded = Math.ceil(this.RECIPE.blood_synthesis_tokens / tokensPerItem);
      const actualCost = itemsNeeded * (item[`${scenario}_price_gp`] / 1_000_000);

      const bestTokensPerItem = best.tokens_received;
      const bestItemsNeeded = Math.ceil(this.RECIPE.blood_synthesis_tokens / bestTokensPerItem);
      const bestActualCost = bestItemsNeeded * (best[`${scenario}_price_gp`] / 1_000_000);
    ...
    // Calculate actual costs (must buy whole items)
    const shardCost = itemsNeededForShards * (cheapestShardItem[`${scenario}_price_gp`] / 1_000_000);
    const tokenCost = itemsNeededForTokens * (cheapestTokenItem[`${scenario}_price_gp`] / 1_000_000);
    ...
    return {
      ...
      shard_component: {
        ...
        cost_per_shard: shardCost / (itemsNeededForShards * shardsPerItem),
      ...
      token_component: {
        ...
        cost_per_token: tokenCost / (itemsNeededForTokens * tokensPerItem),

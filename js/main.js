...
class ArbitrageApp {
   ...
   * Uses empty search_text to fetch ALL trades at once (much faster!)
   */
  async fetchAllTradeData() {
    this.showLoading(true);
    this.updateStatus('Fetching all trade data...');

    this.tradeData = [];

    try {
      console.log('Fetching all trades with empty search (recommended approach)...');

      // Fetch ALL trades at once using empty search_text
      this.updateStatus('Fetching all trades (this may take 1-2 minutes)...');
      this.tradeData = await this.api.fetchTrades('', 100); // Empty string = all trades

      console.log(`Total trades fetched: ${this.tradeData.length}`);

      // Blood diamonds should already be in the all-trades data
      // but we filter them out separately for the bloodchanting calculator

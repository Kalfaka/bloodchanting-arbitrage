...
class ArbitrageApp {
  ...
  /**
   * Extract all unique item names from shop configs
   * @returns {Array<string>} Array of unique item names
   */
  getShopItemNames() {
    const itemNames = new Set();

    // Add items from both shop configs
    for (const shopConfig of this.shopConfigs) {
      for (const item of shopConfig.items) {
        itemNames.add(item.item_name);
      }
    }

    // Add Blood diamond (item_id: 6643) as a special item
    itemNames.add('Blood diamond');

    return Array.from(itemNames);
  }

  /**
   * Fetch trade data for specific shop items only
   * More efficient than fetching ALL trades - only fetches items we care about
   */
  async fetchAllTradeData() {
    this.showLoading(true);
    this.updateStatus('Preparing to fetch trade data...');

    this.tradeData = [];

    try {
      // Get all unique item names from shop configs + Blood diamond
      const itemNames = this.getShopItemNames();
      console.log(`Fetching trades for ${itemNames.length} specific items from shops...`);

      // Progress callback to update status
      const progressCallback = (current, total, itemName) => {
        this.updateStatus(`Fetching trades: ${current}/${total} items (${itemName})...`);
      };

      // Fetch trades for all shop items (max 3 pages per item to avoid CORS proxy rate limits)
      this.tradeData = await this.api.fetchMultipleItems(itemNames, 3, progressCallback);
      ...
      this.bloodDiamondTrades = this.tradeData.filter(trade =>
        trade.item_name.toLowerCase() === 'blood diamond'
      );
